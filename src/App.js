...
function ComponenteSemaforo({ semaforo, numeroSemaforo }) {
  return (
    <div className={`semaforo semaforo-${numeroSemaforo}`}>
      <img 
        src={`${process.env.PUBLIC_URL}/traffic-light-traffic-signal-light-sign-transparent-png-svg-vector-33.png`}
        alt="Semáforo" 
...
function App() {
  const [tiempoReferencia, setTiempoReferencia] = useState(1)
  const [semaforos, setSemaforos] = useState([
    { turno: 1, verde: false, rojo: true, amarillo: false },
    { turno: 2, verde: false, rojo: true, amarillo: false },
    { turno: 3, verde: false, rojo: true, amarillo: false },
    { turno: 4, verde: false, rojo: true, amarillo: false }
  ])

  const workerRef = useRef(null)

  useEffect(() => {
    const worker = new Worker(`${process.env.PUBLIC_URL}/semaforo.worker.js`)
  ...
  return (
    <div className="contenedor-principal">
      ...
      <div className="contenedor-cruce">
        <img src={`${process.env.PUBLIC_URL}/cruce.jpg`} alt="Cruce de calles" className="imagen-cruce" />
